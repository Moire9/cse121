...
#define FLASK_URL "http://10.42.0.1:1234"
...
const uint8_t reset_cmd[] = { 0x80, 0x5D };
const uint8_t  wake_cmd[] = { 0x35, 0x17 };
const uint8_t  meas_cmd[] = { 0x78, 0x66 };
const uint8_t sleep_cmd[] = { 0xB0, 0x98 };
	...
	if (event->event_id == HTTP_EVENT_ON_DATA) {
		if (!esp_http_client_is_chunked_response(event->client)) {
			if (event->user_data) {
				...
				((char*) event->user_data)[len] = 0; // null-terminate
			}
		}
	}
	return ESP_OK;
...
#define ERRCHK(expr) ESP_ERROR_CHECK_WITHOUT_ABORT(expr)
	...
	while (1) {
		...
		LOG("Onboard temperature: %d°C", deg_c);
		...
		const esp_http_client_config_t loc_config = {
			.url = FLASK_URL "/location",
			.user_data = location_buffer,
			.user_agent = "curl/7.68.0",
			.event_handler = http_event_handler,	
		};

		esp_http_client_handle_t get_loc = esp_http_client_init(&loc_config);
		const esp_err_t loc_err = esp_http_client_perform(get_loc);
		esp_http_client_cleanup(get_loc);

		if (loc_err != ESP_OK) {
			ESP_LOGE(TAG, "HTTP GET location failed: %s", esp_err_to_name(loc_err));
			goto end;
		}

		LOG("Server says location is `%s`", location_buffer);

		char url[42];

		sprintf(url, "http://wttr.in/%s?format=3&m&d&T", location_buffer);

		LOG("Querying %s", url);

		// Get weather
		const esp_http_client_config_t weather_config = {
			.url = url,
		...
		esp_http_client_handle_t weather_client = esp_http_client_init(&weather_config);
		const esp_err_t weather_err = esp_http_client_perform(weather_client);
		esp_http_client_cleanup(weather_client);

		if (weather_err != ESP_OK) {
			ESP_LOGE(TAG, "HTTP GET weather failed: %s", esp_err_to_name(weather_err));
			goto end;
		}
		...
		// Post data to server
		char post_data[BUFMAX + 6];

		sprintf(post_data, "%d\n%s", deg_c, response_buffer);

		const esp_http_client_config_t post_config = {
			.url = FLASK_URL,
		...
		esp_http_client_handle_t post = esp_http_client_init(&post_config);
		esp_http_client_set_header(post, "Content-Type", "text/plain");
		esp_http_client_set_post_field(post, post_data, strlen(post_data));

		const esp_err_t post_err = esp_http_client_perform(post);

		esp_http_client_cleanup(post);

		if (post_err != ESP_OK) {
			ESP_LOGE(TAG, "HTTP POST failed: %s", esp_err_to_name(post_err));
			goto end;
		}
